...
//
// Fast routine for sparse tri-linear interpolation of high dimensional
// features. Ignore label is supported.
...
// in_feature.data_ptr<float>(), corner_lut_t.data_ptr<int32_t>(),
// in_worldcoord.data_ptr<float>(), p
template <int TILE_DIM_X, int TILE_DIM_Y, int DUP_X>
__global__ void sp_trilinear_worldcoord_kernel(
    float *__restrict__ out_feature, const float *__restrict__ in_feature,
    const int32_t *__restrict__ corner_lut_t,
    const float *__restrict__ in_worldcoord, SpTrilinear_wc_Params p) {
  ...
  // 0,0,0
  interp_weight[0] = (1.0f - local_coords[0]) * (1.0f - local_coords[1]) *
                     (1.0f - local_coords[2]);
  // 0,0,1
  interp_weight[1] =
      (1.0f - local_coords[0]) * (1.0f - local_coords[1]) * (local_coords[2]);
  // 0,1,0
  interp_weight[2] =
      (1.0f - local_coords[0]) * (local_coords[1]) * (1.0f - local_coords[2]);
  // 0,1,1
  interp_weight[3] =
      (1.0f - local_coords[0]) * (local_coords[1]) * (local_coords[2]);
  // 1,0,0
  interp_weight[4] =
      (local_coords[0]) * (1.0f - local_coords[1]) * (1.0f - local_coords[2]);
  // 1,0,1
  interp_weight[5] =
      (local_coords[0]) * (1.0f - local_coords[1]) * (local_coords[2]);
  // 1,1,0
  interp_weight[6] =
      (local_coords[0]) * (local_coords[1]) * (1.0f - local_coords[2]);
  // 1,1,1
  interp_weight[7] = (local_coords[0]) * (local_coords[1]) * (local_coords[2]);

  int indices[8];
  // Hard boundary check (zero padding)
  if (isnan(world_coords[0]) || isnan(world_coords[1]) ||
      isnan(world_coords[2])) {
    indices[0] = -1;
    indices[1] = -1;
    indices[2] = -1;
    indices[3] = -1;
...
#pragma unroll
    for (int j = 0; j < 8; j++) {
      if (indices[j] >= 0) {
        interp_feat = fmaf(in_feature[indices[j] * p.in_feature_dim + idx_feat],
                           interp_weight[j], interp_feat);
      }
    }
...
// sp_trilinear_worldcoord_backward2feature_kernel<TILE_DIM_X, TILE_DIM_Y,
// DUP_X><<<dimGrid, dimBlock, 0, stream>>>(
//         in_feature_grad.data_ptr<float>(),
//         out_feature_grad.data_ptr<float>(), in_feature.data_ptr<float>(),
//         in_corner_lut.data_ptr<int32_t>(), in_worldcoord.data_ptr<float>(), p
//  Backward to feature
template <int TILE_DIM_X, int TILE_DIM_Y, int DUP_X>
__global__ void sp_trilinear_worldcoord_backward2feature_kernel(
    float *__restrict__ in_feature_grad,
    const float *__restrict__ out_feature_grad,
    const int32_t *__restrict__ corner_lut_t,
    const float *__restrict__ in_worldcoord, SpTrilinear_wc_Params p) {
...
#pragma unroll
  ...
  for (int i = 0; i < DUP_X; i++) {
    ...
    float grad =
        out_feature_grad[idx_out_feature + stride_out_feature * idx_feat];
#pragma unroll
    for (int j = 0; j < 8; j++) {
      if (indices[j] >= 0) {
        // indices[j]*p.in_feature_dim+idx_feat
        atomicAdd(&in_feature_grad[indices[j] * p.in_feature_dim + idx_feat],
                  grad * interp_weight[j]);
      }
    }
    // idx_feat += TILE_DIM_X;
    idx_feat += TILE_DIM_X * GRID_X;
...
//      ---Index:          int32   [..., 8], containing [0, M]. 0 is ignore
//      label.
//      ---Coord:          float32 [..., 3]
// Output:
//      Interp. Feat:   float32 [..., C]
// std::vector<torch::Tensor>
torch::Tensor sp_trilinear_worldcoord_cuda(const torch::Tensor &in_feature,
                                           const torch::Tensor &in_corner_lut,
                                           const torch::Tensor &in_worldcoord,
                                           bool ign_zero, int channel_pos) {
  ...
  // CHECK_CONTIGUOUS(in_corner_lut); // Will still run correctly, but
  // performance will suffer. CHECK_CONTIGUOUS(in_worldcoord);
  ...
  torch::Tensor out_feature = torch::empty(
      out_feature_shape,
  ...
  //    }
  //    out_feature_shape.push_back(p.in_feature_dim);
  //    out_feature = torch::empty(out_feature_shape,
  //    torch::TensorOptions().dtype(torch::kFloat32).device(device));
  //}
  ...
  // dim3 dimGrid((p.in_feature_dim+(TILE_DIM_X*DUP_X)-1)/(TILE_DIM_X*DUP_X),
  // (numentries+TILE_DIM_Y-1)/TILE_DIM_Y, 1);
  dim3 dimGrid(
      (numentries + TILE_DIM_Y - 1) / TILE_DIM_Y,
      (p.in_feature_dim + (TILE_DIM_X * DUP_X) - 1) / (TILE_DIM_X * DUP_X), 1);
  dim3 dimBlock(TILE_DIM_X, TILE_DIM_Y, 1);

  sp_trilinear_worldcoord_kernel<TILE_DIM_X, TILE_DIM_Y, DUP_X>
      <<<dimGrid, dimBlock, 0, stream>>>(out_feature.data_ptr<float>(),
                                         in_feature.data_ptr<float>(),
                                         in_corner_lut.data_ptr<int32_t>(),
                                         in_worldcoord.data_ptr<float>(), p);
...
//      in_feature:         float32 [M, C]
//      in_corner_lut:      int32   [X Y Z]
//      ---in_index:        int32   [..., 8], containing [0, M]. 0 is ignore
//      label. in_worldcoord:      float32 [..., 3] ign_zero:           bool
//      need_coord_grad:    bool
// Output:
//      in_feature_grad:    float32 [M, C]
//      in_coord_grad:      float32 [..., 3]
std::vector<torch::Tensor> sp_trilinear_worldcoord_backward_cuda(
    const torch::Tensor &out_feature_grad, const torch::Tensor &in_feature,
    const torch::Tensor &in_corner_lut, const torch::Tensor &in_worldcoord,
    bool ign_zero, bool need_coord_grad) {
  ...
  torch::Tensor in_feature_grad = torch::zeros(
      {p.in_feature_numentries, p.in_feature_dim},
      torch::TensorOptions().dtype(torch::kFloat32).device(device));
...

  {
    ...
    const int TILE_DIM_Y = 64; // entry dim
    const int DUP_X = 4;       // To amortize the cost of weight computation
    dim3 dimGrid((p.in_feature_dim + (TILE_DIM_X * DUP_X) - 1) /
                     (TILE_DIM_X * DUP_X),
                 (numentries + TILE_DIM_Y - 1) / TILE_DIM_Y, 1);
    dim3 dimBlock(TILE_DIM_X, TILE_DIM_Y, 1);
    ...
    sp_trilinear_worldcoord_backward2feature_kernel<TILE_DIM_X, TILE_DIM_Y,
                                                    DUP_X>
        <<<dimGrid, dimBlock, 0, stream>>>(in_feature_grad.data_ptr<float>(),
