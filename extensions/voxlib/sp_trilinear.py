...
class SparseTrilinearWorldCoordFunction(Function):
    ...
    def forward(ctx, in_feature, corner_lut_t, in_worldcoord, ign_zero):
        out_feature = voxlib.sp_trilinear_worldcoord(
            in_feature, corner_lut_t, in_worldcoord, ign_zero, -1
        )
...
def sparse_trilinear_interp_worldcoord(
    in_feature, corner_lut_t, in_worldcoord, ign_zero=False
):
    return SparseTrilinearWorldCoordFunction.apply(
        in_feature, corner_lut_t, in_worldcoord, ign_zero
    )
