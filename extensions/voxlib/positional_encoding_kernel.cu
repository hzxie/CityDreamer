...
// dim3 dimGrid((p.post_size+TILE_DIM_X-1)/TILE_DIM_X,
// (p.pre_size+TILE_DIM_Y-1)/TILE_DIM_Y, 1); dim3 dimBlock(TILE_DIM_X,
// TILE_DIM_Y, 1);
template <int TILE_DIM_X, int TILE_DIM_Y, int DUP_Y>
__global__ void positional_encoding_kernel(float *__restrict__ out_feature,
                                           const float *__restrict__ in_feature,
                                           const PE_Params p) {

  const int idx_feat = blockIdx.x * TILE_DIM_X + threadIdx.x;
  const int idx_entry_base =
      blockIdx.y * TILE_DIM_Y * DUP_Y + threadIdx.y * DUP_Y;
  ...
  for (int j = 0; j < DUP_Y; j++) {
    ...
    for (int i = 0; i < p.ndegrees; i++) {
      ...
      out_feature[idx_entry * p.post_size * stride + i * 2 * p.post_size +
                  idx_feat] = sinrad;
      out_feature[idx_entry * p.post_size * stride + (i * 2 + 1) * p.post_size +
                  idx_feat] = cosrad;
    }
    if (p.incl_orig) {
      out_feature[idx_entry * p.post_size * stride +
                  (stride - 1) * p.post_size + idx_feat] = data;
    }
  }
}
...
template <int TILE_DIM_X, int TILE_DIM_Y, int DUP_Y>
__global__ void
positional_encoding_backward_kernel(float *__restrict__ in_feature_grad,
  ...
  for (int j = 0; j < DUP_Y; j++) {
    ...
    for (int i = 0; i < p.ndegrees; i++) {
      ...
      grad_t =
          out_feature_grad[idx_entry * p.post_size * stride +
                           i * 2 * p.post_size + idx_feat] *
          out_feature[idx_entry * p.post_size * stride +
                      (i * 2 + 1) * p.post_size + idx_feat]; // cos(x*pi*(2^i))

      grad_t -=
          out_feature_grad[idx_entry * p.post_size * stride +
                           (i * 2 + 1) * p.post_size + idx_feat] *
          out_feature[idx_entry * p.post_size * stride + (i * 2) * p.post_size +
                      idx_feat]; // -sin(x*pi*(2^i))

      grad += grad_t * CUDART_PI_F * exp2f(i);
    }
    if (p.incl_orig) {
      grad += out_feature_grad[idx_entry * p.post_size * stride +
                               (stride - 1) * p.post_size + idx_feat];
    }
...
//      incl_orig:      bool    Whether to include original feature vector or
//      not
// Output:
...
torch::Tensor positional_encoding_cuda(const torch::Tensor &in_feature,
  ...
  // Launch CUDA kernel
  // Case 1: Concat at the last dimension (post_size < pre_size)  -->  Each
  // thread handle a single post_size Case 2: Concat at the middle (post_size >
  // pre_size)  -->  Each thread handle
  const int TILE_DIM_X = 16; // channel dim
  const int TILE_DIM_Y = 64; // entry dim
  // const int DUP_Y = 4; // Each thread handle multiple entries to save threads
  const int DUP_Y = 8; // DGXA 64 samples per ray @ 256x256
  dim3 dimGrid((p.post_size + TILE_DIM_X - 1) / TILE_DIM_X,
               (p.pre_size + (TILE_DIM_Y * DUP_Y) - 1) / (TILE_DIM_Y * DUP_Y),
               1);
  dim3 dimBlock(TILE_DIM_X, TILE_DIM_Y, 1);
  positional_encoding_kernel<TILE_DIM_X, TILE_DIM_Y, DUP_Y>
...
// in_feature_grad = voxrender_op.positional_encoding_backward(out_feature_grad,
// out_feature, ctx.pe_degrees, ctx.dim, ctx.incl_orig);
//  Input:
//       out_feature_grad:   float32 [..., N*ndegree*2+incl_orig, ...]
//       out_feature:        float32 [..., N*ndegree*2+incl_orig, ...]
//       ndegrees:           int32   Degrees of PE encoding
//       dim:                int32   Dimension to concatenate
//       incl_orig:          bool    Whether to include original feature vector
//       or not
//  Output:
//       in_feature_grad:    float32 [..., N, ...]
//  std::vector<torch::Tensor>
torch::Tensor
positional_encoding_backward_cuda(const torch::Tensor &out_feature_grad_,
  ...
  // Always produce contiguous output
  torch::Tensor in_feature_grad = torch::empty(
      out_feature_shape,
      torch::TensorOptions().dtype(torch::kFloat32).device(device));
  ...
  dim3 dimGrid((p.post_size + TILE_DIM_X - 1) / TILE_DIM_X,
               ...
               1);
  dim3 dimBlock(TILE_DIM_X, TILE_DIM_Y, 1);
  positional_encoding_backward_kernel<TILE_DIM_X, TILE_DIM_Y, DUP_Y>
